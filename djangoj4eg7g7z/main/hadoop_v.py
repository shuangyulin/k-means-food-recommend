...
def upload_csv_mapreduce_hadoop():
    # 查询数据
    query = "SELECT * FROM meishiinfo"
    df = pd.read_sql(query, connection)
    local_csv_path = os.path.join(parent_directory,"meishiinfo.csv")
    # 导出为 CSV 文件
    df.to_csv(local_csv_path, index=False)
    print(f"数据成功导出到 CSV 文件: {local_csv_path}")
    hdfs_csv_path = '/input/meishiinfo.csv'
    # 目标 HDFS 路径
...
def send_cmd():
    ...
    [
        ...
        "-mapper", f"\"python value_mapper.py {csv_index('meishiinfo.csv','shopname')} {csv_index('meishiinfo.csv','score')} 无  \"",
        "-reducer", "\"python value_reducer.py shopname\"",
        "-input", "hdfs://localhost:9000/input/meishiinfo.csv",
        "-output", "hdfs://localhost:9000/output/meishiinfo/valueshopnamescore"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/value_mapper.py,hdfs://localhost:9000/input/value_reducer.py\"",
        "-mapper", f"\"python value_mapper.py {csv_index('meishiinfo.csv','shopname')} {csv_index('meishiinfo.csv','qsprice')} 无  \"",
        "-reducer", "\"python value_reducer.py shopname\"",
        "-input", "hdfs://localhost:9000/input/meishiinfo.csv",
        "-output", "hdfs://localhost:9000/output/meishiinfo/valueshopnameqsprice"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/value_mapper.py,hdfs://localhost:9000/input/value_reducer.py\"",
        "-mapper", f"\"python value_mapper.py {csv_index('meishiinfo.csv','msname')} {csv_index('meishiinfo.csv','monthsales')} 无  \"",
        "-reducer", "\"python value_reducer.py msname\"",
        "-input", "hdfs://localhost:9000/input/meishiinfo.csv",
        "-output", "hdfs://localhost:9000/output/meishiinfo/valuemsnamemonthsales"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/value_mapper.py,hdfs://localhost:9000/input/value_reducer.py\"",
        "-mapper", f"\"python value_mapper.py {csv_index('meishiinfo.csv','msname')} {csv_index('meishiinfo.csv','jiage')} 无  \"",
        "-reducer", "\"python value_reducer.py msname\"",
        "-input", "hdfs://localhost:9000/input/meishiinfo.csv",
        "-output", "hdfs://localhost:9000/output/meishiinfo/valuemsnamejiage"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/group_mapper.py,hdfs://localhost:9000/input/group_reducer.py\"",
        "-mapper", f"\"python group_mapper.py {csv_index('meishiinfo.csv','shopname')}\"",
        "-reducer", "\"python group_reducer.py shopname\"",
        "-input", "hdfs://localhost:9000/input/meishiinfo.csv",
        "-output", "hdfs://localhost:9000/output/meishiinfo/groupshopname"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/group_mapper.py,hdfs://localhost:9000/input/group_reducer.py\"",
        "-mapper", f"\"python group_mapper.py {csv_index('meishiinfo.csv','msname')}\"",
        "-reducer", "\"python group_reducer.py msname\"",
        "-input", "hdfs://localhost:9000/input/meishiinfo.csv",
        "-output", "hdfs://localhost:9000/output/meishiinfo/groupmsname"
    ],
    ]
