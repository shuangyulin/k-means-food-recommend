		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"10px 0","margin":"0 0px 0px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"auto","padding":"0","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">店名</label>
						<el-input v-model="searchForm.shopname" placeholder="店名" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"width":"auto","padding":"0","margin":"0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('meishiinfoforecast','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('meishiinfoforecast','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"0 0px","fontSize":"13px","color":"#000"}'>
				<el-table class="tables"
					...
					:border='false'
					v-if="isAuth('meishiinfoforecast','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('meishiinfoforecast','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('meishiinfoforecast','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('meishiinfoforecast','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
		...
		methods: {
			...
			forecastClick(row) {
				this.$confirm(`是否进行数据预测?`, "提示", {
					...
					let arr = 'shopname,score,monthsales,jiage'.split(',')
					let brr = {
						id: row.id
					}
					for(let x in arr){
			...
			forecastImgClick(){
				...
				this.$http({
					url: 'meishiinfoforecast/forecastimgs',
					method: 'get',
				}).then(res=>{
					if (loading) loading.close()
					if(res.data&&res.data.code==0){
