	...
	<div class="home-preview">
		...
		<div id="animate_newsnews" class="news animate__animated">
			<div class="news_title_box">
				<span class="news_title">校园资讯</span>
		...
		<!-- 新闻资讯 -->
		<!-- 商品推荐 -->
		<div id="animate_recommendmeishiinfo" class="recommend animate__animated">
			<div class="recommend_title_box">
				<span class="recommend_title">美食信息推荐</span>
				<span class="recommend_subhead">{{'meishiinfo'.toUpperCase()}} RECOMMEND</span>
			</div>
			<div v-if="meishiinfoRecommend.length" class="list list12 index-pv1 list12meishiinfo">
				<div class="list-body">
					<div class="item list-item animation-box" v-for="item,index in meishiinfoRecommend" :key="index" @click="toDetail('meishiinfoDetail', item)">
						...
						<div class="name">{{item.msname}}</div>
					</div>
				</div>
			</div>
			<div class="moreBtn" @click="moreBtn('meishiinfo')">
				<span class="text">查看更多</span>
				<i class="icon iconfont icon-gengduo1"></i>
			</div>
		</div>
		...
		methods: {
			...
			handleScroll() {
				let arr = [
					...
					{id:'animate_recommendmeishiinfo',css:'animate__backInRight'},
			...
			getList() {
				let autoSortUrl = "";
				let data = {}
				autoSortUrl = "meishiinfo/autoSort";
				if(localStorage.getItem('frontToken')) {
					autoSortUrl = "meishiinfo/autoSort2";
				}
				data = {
					page: 1,
					limit: 8,
				...
				this.$http.get(autoSortUrl, {params: data}).then(res => {
					if (res.data.code == 0) {
						this.meishiinfoRecommend = res.data.data.list;
					}
				});
